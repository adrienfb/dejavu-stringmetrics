...
public class RatcliffObershelpMetric implements SimilarityMetric {
	...
	private static final int lcs(String s1, int s1min, int s1max, String s2, int s2min, int s2max) {
		// stop criteria
		if (s1min < 0 || s1max > s1.length() || s1max-s1min < 1 || s2min < 0 || s2max > s2.length() || s2max-s2min < 1)
			return 0;
		...
		while (i>0 && j>0) {
			if (s1.charAt(s1min+i-1) == s2.charAt(s2min+j-1)) {
				if (s1min+i-1 < s1LCSmin) s1LCSmin = s1min+i-1;
				if (s1LCSmax < s1min+i-1) s1LCSmax = s1min+i;

				if (s2min+j-1 < s2LCSmin) s2LCSmin = s2min+j-1;
				if (s2LCSmax < s2min+j-1) s2LCSmax = s2min+j;

				i--;
				j--;
			} else {
				if (array[i-1][j] > array[i][j-1]) { i--; }
				else if (array[i-1][j] < array[i][j-1]) { j--;}
				else { i--; }
			}
		}
	...
	/**
	 * @return	standard RatcliffObershelp distance object that can be used to determine the distance between two strings
	 * @see JaroMetric
	 */
