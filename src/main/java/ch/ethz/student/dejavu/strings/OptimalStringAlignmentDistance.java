 ...
 * deletion, or substitution of a single character. The Optimal String Alignment distance is an
 * extension of Levenshtein distance were it treats adjacent transpositions as another basic
 * operation
 *
  ...
  public double computeDistance(String s1, String s2) {
    ...
    for (int i = 1; i <= s1.length(); i++) {
      for (int j = 1; j <= s2.length(); j++) {
        ...
        if (i > 1 && j > 1 && s1.charAt(i - 1) == s2.charAt(j - 2) && s1.charAt(i - 2) == s2
            .charAt(j - 1)) {
