 ...
 * 
 * This implementation operates on char-bigrams and follows the definition given on Wikipedia. Given the set of bi-grams S1 and S2 extracted
 * from input strings s1 and s2 respectively the similarity is computed according to the formula:
 * 
 * <p>sim(s1, s2) = 2 * (S1 intersect S2 ) / ( size(S1) + size(S2) )</p>
 * 
 * <p>If you would like to use this distance with another base as character bi-grams please refer to the same named class in the vectors package!</p>
 * 
...
public class DiceCoefficientMetric implements SimilarityMetric {
	...
	/**
	 * @return	standard Dice Coefficient distance object that can be used to determine the distance between two strings
	 * @see DiceCoefficientMetric
	 */
