...
/**
 * The Damerau-Levenshtein distance between two strings is given by counting the minimum number of
 * operations needed to transform one string into the other, where an operation is defined as an
 * insertion, deletion, or substitution of a single character, or a transposition of two adjacent
 * characters.
 *
    ...
    for (int i = 1; i <= s1.length(); i++) {
      ...
      for (int j = 1; j <= s2.length(); j++) {
        ...
        if (s1.charAt(i - 1) == s2.charAt(j - 1)) {
          ...
          distance[i + 1][j + 1] =
              Utilities.min(distance[i][j], distance[i + 1][j], distance[i][j + 1]) + 1;
        }
        distance[i + 1][j + 1] =
            Math.min(distance[i + 1][j + 1], distance[i1][j1] + (i - i1 - 1) + 1 + (j - j1 - 1));
      }
