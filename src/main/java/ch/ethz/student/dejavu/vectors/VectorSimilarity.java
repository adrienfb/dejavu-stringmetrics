...
public abstract class VectorSimilarity {
	...
	protected HashMap<Object, Integer> getTokenVectorTokenizer(String s, HashMap<Object, Integer> vec) {
		if (vec == null) {
			vec = new HashMap<Object, Integer>();
		} else {
			vec.clear();
	...
	private HashMap<Object, Integer> getSparatorTokenVector(String s, StringTokenizer tokenizer, HashMap<Object, Integer> vec) {
	 ...
	 *
	 *	<p><code>VectorBuilder</code> is an abstract class that can set vector similarity parameters.</p>
	 *	<p>Usage of a Builder in general (e.g. JaccardDistance):</p>
	 *	<p>Builder b = JaccardDistance.getBuilder();</br>
	 *	b.build();</br></p>
	 *	
	 *	<p>If options should be set the methods can be called:</br>
	 *	b.granularity(Granularity.CHAR_BASED)</br>
	 *	More options are described in the parameter section.</p>
	 *
	 *	<p>Of cause the parameter calls can be chained:</br>
	 *	JaccardDistance d = JaccardDistance.getBuilder()</br>
	 *		.granularity(Granularity.SEPARATOR_BASED)</br>
	 *		.delimiter(",").build();</p>
	 *
	 * @author Florian Froese
	 ...
	 * @param takeTokenizer	internal tokenizing of string: If true a Subclass of Tokenizer is used, otherwise an internal faster implementation is used
	 * @param takeKeyset	internal handling of vectors: If true a HashSet is used, otherwise a HashMap. (For performance reasons in some similarities)
	 * @since 1.0
	 */
	public abstract static class VectorBuilder<E> {
		...
		/**
		 * Sets the number of characters that should form an ngram.
		 * Thus if nGram=2 the word "hello" is split into ["he","el","ll","lo"]
		 * 
		...
		/**
		 * Optional tokenizer that can be used.
		 * Here a tokenizer can be specified that must inherit the {@link Tokenizer} class.
		...
		/**
		 * If this option is set to true a HashSet is generated if similarity(s1,s2) is called. 
		 * Thus only the tokens and not their frequency are stored.
		 * Otherwise a HashMap is used and the occurences of a token are counted in the value field.
		 * @param b	boolean value true/false
		 * @return	this builder object
		 */
