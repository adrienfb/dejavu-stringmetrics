...
/**
 * Tversky Index implementation based on a vector model. This implementation uses the formula:
 * tverskyIndex = intersect(A, B) / ( intersect(A,B) + a * |A - B| + b * |B - A| ) where A and B are
 * sets of tokens. </br> |.| is the size of the token set and A - B denotes the relative complement
 * of B in A.</br> e.g. a = 0.5 and b = 0.5 produces the Dice Coefficient</br> and a = 1 and b = 1
 * produces the Tanimoto Coefficient
 *
 * <p>For creation please use default TverskyIndex or the Builder {@link Builder} provided by the
 * static methods.</p>
 *
...
public class TverskyIndex extends VectorSimilarity implements SimilarityMetric {
  ...
  /**
   * TverskyIndex Builder Restriction: alpha and beta parameters must both be greater or equal to
   * 0!
   *
  ...
  public static class Builder extends VectorBuilder<Builder> {
    ...
    /**
     * Sets the alpha parameter of tversky index. Has to be greater or equal to 0!
     *
    ...
    /**
     * Sets the beta parameter of tversky index. Has to be greater or equal to 0!
     *
