...
/**
 * Overlap Coefficient implementation based on a vector model. This implementation uses the formula:
 * overlapCofficient = intersect(A, B) / ( Min(|A|, |B|) ) where A and B are sets of tokens. </br>
 * |.| is the size of the token set.
 *
 * <p>For creation please use default OverlapCoefficient or the Builder {@link Builder} provided by
...
public class OverlapCoefficient extends VectorSimilarity implements SimilarityMetric {
  ...
  /**
   * @return standard overlap coefficient that can be used to determine the overlap coefficient of
   * two strings.
   * @see OverlapCoefficient
   */
  ...
  /**
   * OverlapCoefficient Builder No further restrictions or additional parameters.
   *
